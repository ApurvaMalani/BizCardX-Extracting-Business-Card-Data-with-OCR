...
st.set_page_config(page_title="BizCardX: Extracting Business Card Data with OCR",
                   ...
                   menu_items={'About': """# This OCR app is created for a project!"""})
...
def setting_bg():
    st.markdown(f""" <style>.stApp {{
                        background: url("https://hougumlaw.com/wp-content/uploads/2016/05/light-website-backgrounds-light-color-background-images-light-color-background-images-for-website-1024x640.jpg");
...
mydb = sql.connect(host="localhost",
                   user="root",
                   password="Pass@1234", auth_plugin='mysql_native_password'
                   )
...
if selected == "Upload & Extract":
    ...
    uploaded_card = st.file_uploader("Upload card here in png/jpeg/jpg format", label_visibility="collapsed", type=["png", "jpeg", "jpg"])
    ...
    if uploaded_card is not None:
        ...
        if st.button("Show Data"):
            ...
            with col_1:
                st.text_input('Name', data["card_holder"][0])
                st.text_input('Designation', data["designation"][0])
                st.text_input('Company name', data["company_name"][0])
                st.text_input('Mobile', data["mobile_number"][0])

            with col_2:
                st.text_input('Email', data["email"][0])
                st.text_input('Website', data["website"][0])
                st.text_input('Address', data["area"]+data['city']+data['state'])
                st.text_input('Pincode', data["pin_code"][0])
